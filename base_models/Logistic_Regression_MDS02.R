...
#fs_control <- rfeControl(functions=rfFuncs, method="cv", number=10)
#results <- rfe(train_data[, independent_variables], train_data$Endpoint, sizes = c(1:ncol(train_data) - 1), rfeControl = fs_control) # nolint
#print(results)
...
roc_imp <- filterVarImp(x = Table1[,0:27], y = Table1$Endpoint)
