...
start_pos <- 1
subset <- majority_data[start_pos:ceiling(majority_size/2),]
subset <- rbind(subset, minority_data)
balanced_datasets <- append(balanced_datasets, list(subset))
start_pos <- start_pos + floor(majority_size/2) + 1
...
for (data in balanced_datasets){
    ...
    train_smote <- SMOTE(data[, -which(colnames(Table1) == "Endpoint")], data$Endpoint, K=5)
    data <- train_smote$data
    data$class <- factor(data$class)
    names(data)[names(data) == "class"] <- "Endpoint"
