...
value_box_choices <- c(
  ...
  "Age (age)" = "age",
  "Gender (sex)" = "sex",
  "Number of Lymph Node Harvested (lnn)" = "lnn",
  "Taxane-based Chemotherapy (tax)" = "tax",
  "Radiation Fraction (fx)" = "fx",
  "Amount of Radiation - Gray (Gy)" = "Gy",
  "Breast Reconstruction (recon)" = "recon",
  "Chemotherapy (che)" = "che",
  "Axilla Radiation Therapy (axi)" = "axi",
  "Platelets (PLT)" = "PLT",
  ...
  "Monocyte" = "Monocyte",
  "Hematocrit (Hct)" = "Hct",
  "Segmented Neutrophil (Segmented.neutrophil)" = "Segmented.neutrophil",
  "Mean Corpuscular Hemoglobin Concentration (MCHC)" = "MCHC",
  "Hemoglobin (Hb)" = "Hb",
  "Lymphocyte" = "Lymphocyte",
  "Mean Corpuscular Volume (MCV)" = "MCV",
  "Mean Corpuscular Hemoglobin (MCH)" = "MCH",
  "Potassium Serum (Potassium.serum)" = "Potassium.serum",
  "Chloride Serum (Chloride.serum)" = "Chloride.serum",
...
server <- function(input, output) {
  ...
  output$pred.single <- renderValueBox({
    ...
    validate(need(all(DataTable[["recon"]] %in% c(0, 1, 2)), ""))
    validate(need(all(DataTable[["tax"]] %in% c(0, 1, 2)), ""))
    validate(need(all(DataTable[["che"]] %in% c(0, 1)), ""))
    validate(need(all(DataTable[["axi"]] %in% c(0, 1)), ""))
