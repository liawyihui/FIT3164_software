...
# train the model
  ...
  NNetmodel <- train(Endpoint~., data=train, method="pcaNNet", trControl=control, tuneLength=5)
# summarize the model
  print(NNetmodel)
# save model for later use
  save(NNetmodel, file= paste(DataFolder,"/NNetmodel.Rdata", sep = ""))
...
# Get probability
  pred_all_prob <- as.data.frame(NNetmodel %>% predict(Table1, type = "prob"))
...
# Make prediction on test set 
  pred_train_prob <- as.data.frame(NNetmodel %>% predict(train, type = "prob"))
  pred_test_prob <- as.data.frame(NNetmodel %>% predict(test, type = "prob"))
  pred_all_prob <- as.data.frame(NNetmodel %>% predict(Table1, type = "prob"))
...
# Confusion matrix and model performance
  union <- union(pred_all, Table1$Endpoint)
  Table2 <- table(factor(pred_all,union), factor(Table1$Endpoint,union))
  ConfMat <- confusionMatrix(Table2)
  Performance <- setDT(as.data.frame(ConfMat$byClass), keep.rownames = TRUE)[]
...
# Estimate Descriptor importance
  DescImportance <- data.frame(Descriptor = row.names(varImp(NNetmodel, scale=TRUE)$importance),
                               Value = varImp(NNetmodel, scale=TRUE)$importance$X0)
